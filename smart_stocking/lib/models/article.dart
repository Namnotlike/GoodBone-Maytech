...
class Article {
  ...
  factory Article.fromJson1(Map<String, dynamic> json) {
    return Article(
      id: json['id'],
      title: json['title']?['rendered'] ?? '',
      content: json['content']?['rendered'] ?? '',
      image: json['yoast_head_json']?['og_image']?[0]?['url'] ?? WpConfig.defaultFeatureImage, // fallback nếu chưa xử lý media
      authorId: json['author'] ?? 0,
      date: json['date_gmt'] != null
          ? DateTime.parse("${json['date_gmt']}Z").toLocal()
          : null,
      link: json['link'] ?? '',
      tags: json['tags'] != null ? List<int>.from(json['tags']) : [],
      featured: json['sticky'] ?? false,
      author: '',
      avatar: '',
      category: '',
      catId: 0,
